      ...
      * SONAR TEST DATA AREA (NLOPEZ)
       01 SONAR-TEST-DATA.
           05 ID-FIELD     PIC X(20).
           05 SQL-STR      PIC X(20).

           05 USER-PARAM   PIC X(10).
           05 USER-CMD     PIC X(10).
           
           05 SSN          PIC X(13) VALUE '123-45-6789'.           
           05 DB-USER   PIC X(20) VALUE "APPUSER".
           05 DB-PASS   PIC X(20) VALUE "Pa$$w0rd123".
      ...
      *    COPY DFHEIBLK.
       ...
       EPSCMORT-MAINLINE. 
           DISPLAY 'NJL-TRACE: Init (see CEEMSG DD) v4.1 !'
      ...
      * NLOPEZ - SS test Code                
                      PERFORM SEC-1000-SONAR-RULE-BREAK
      ...
      *      PERFORM A805-DUMMY-SQL-CALL.
       ...
       A805-DUMMY-SQL-CALL.
           ...
           MOVE 'DB2 Access Sucessful v2' TO MSGERRO.
           IF SQLCODE NOT = 100 and not = 0 
               MOVE 'SQL Err on SYSDUMMY1' TO MSGERRO    
               display MSGERRO 'SQLERR: ' SQLCODE
           END-IF.

       SEC-1000-SONAR-RULE-BREAK.
      * simulate some common coding violations picked up by Sonar 
      *  
      * sql injections  
                     
      *     STRING "SELECT * FROM EMP WHERE ID=" ID-FIELD INTO SQL-STR.
      *     EXEC SQL PREPARE STMT FROM :SQL-STR END-EXEC.
      *     EXEC SQL EXECUTE STMT END-EXEC.
      
      * simulate Dynamic CALL to external programs with untrusted data 
      *     STRING "MYPROG " USER-PARAM INTO USER-CMD
           CALL USER-CMD.
