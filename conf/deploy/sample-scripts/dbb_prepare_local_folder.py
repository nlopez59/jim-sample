...
##NJL Refresh from Prod Guide 2/28/26
# Mod- added new DeployType DBRMP 
# Desc: IBM sample python script to add DBB Deploy Types to artifact filename extension 
# ref: https://www.ibm.com/docs/en/developer-for-zos/16.0.x?topic=deployment-static-python-translator
# Note: Wazi Deploy under IDZee V17 replaced this python script with a new DBB zBuilder 'package' step.
###
...
class DBBUtilities(object):
    ...
    def get_copy_mode(deployType:str = "LOAD", **kwargs) -> str:
        ...
        if re.search('LOAD', deployType, re.IGNORECASE):
            return "LOAD"
        elif re.search('DBRMP', deployType, re.IGNORECASE):
            return "BINARY"        
